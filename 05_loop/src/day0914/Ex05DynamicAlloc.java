...
public class Ex05DynamicAlloc {
   ...
   static int[] add(int[] arr, int element) {
      ...
      // int[] arr = new int[4] 
      // 해당 배열의 가장 마지막 index 번호는 몇번인가? 마지막 index 번호는 크기 -1
      // 3
      // 현재 크기는?
      // 4
   ...
   static boolean contains(int[] arr, int element) {
	   //for문을 이용해서
	   //arr의 각인덱스와 element를 비교해서 같으면 return true를 한다.
	   for(int i = 0; i < arr.length; i++) { // i:0
		   if(arr[i] == element) {			//	i < arr.length true
			   //i번째 인덱스와 element의값이 같으므로
			   //return true 해준다.
			   return true;
		   }
	   }
   ...
   static int indexOf(int[]arr, int element) {
	   ...
	   //만약 element가 arr에 존재 하지 않는다면
	   //우리가 어떤값을 리턴해주어야할까?
	   //인덱스는 무조건 0부터 arr의크기 - 1 이기 때문에
	   //양의 정수로 우리가 존재하지 않는다는 표시해주기 힘들다.
	   //따라서 음의 정수중에서 가장 간단한 "-1" 을 리턴해서
	   //만약 배열에 해당하는 엘리멘트가 존재하지 않으면
	   //인덱스는 -1을 리턴해주면 된다.
	   
	   //for문을 돌려서
	   //만약 i번째 인덱스 값이element와 같으면
	   //i를 리턴해주면 된다. //배열의 앞의 인덱스는 언제나 0
   ...
   public static void main(String[] args) { //length 로는 이 배열의 총크기밖에 알수없다
	   int[] arr2 = new int[5];
	   arr2[0] = 10;
      ...
      //메소드에서 return이 실행되는 순간 메소든느 거기서 끝나는건가요? 넵
      
      //contains 메소드 실행한 결과값
   ...
   // 그렇다면 우리가 배열에 요소를 삭제하면? 크기가 줄어야한다
   ...
   static int[] removeByIndex(int[] arr, int index) {
	   ...
	   //인덱스가 가능한 범위는 몇부터 몇인가? 0부터 arr.length - 1 (0부터 크기 -1)
	   if(index < 0 || index >= arr.length) {
		   return arr;
	   }
	   
	   //배열의 값을 임시 보관할 temp를 만들어주자
	   //temp의 크기는 얼마인가? arr.length
	   ...
	   // arr을 새로 초기화해주자
	   // 이때arr의 크기는 얼마인가?
	   arr = new int[arr.length - 1]; //사이즈 하나 줄임
	 ...
	 //index = 5         0   1  2 3  4  6  7
	   ...
	   return arr;
   		...
   		static int[] removeByElement(int[] arr, int element) {
   			//사실 이 메소드는 우리가 이존에 만들어놨던 메소드를 재활용하면 매우 쉽게 만들어 줄 수 있다.
...
//arr = new int[arr.length - 1];이 부분이요
//arr = new int[size+1]; 랑 대비되는 건가요? 네
