...
public class Ex01String {
	public static void main(String[] args) throws IOException {
		String string1 = new String("abc");//1번
		String string2 = "abc";				//2번	이 둘은 차이가 없다.
		...
		// null != ""; true
		// null 인 상태에선 아무것도 할 수 없다. 새로운 초기화를 할때까지
		...
		// 접근할 수 없게 되는 것이다.
		// 이사가려고 주소지 등록은 했는데 건물도 안올라간 상태
		
		//즉 isEmpty() 메소드는
		// 해당 스트링이 널이 아닐 때만 실행 가능하다.
		
		
		System.out.println("string1.isEmpty(): "+string1.isEmpty());
		System.out.println("\"\".isEmpty(): "+"".isEmpty());
		
		//5번: 해당 스트링의 길이를 알고 싶을때에는 length()
		System.out.println("string1.length(): "+string1.length()); //abc 세글자라 3
		...
		// 		해당 스트링이 모두다 스트링으로 이루어졌는지 체크할 때에는 \d* 으로 테스트 한다.
		System.out.println("string1.matches(\\d*): "+string1.matches("\\d*")); // *은 하나 이상인지 체크
